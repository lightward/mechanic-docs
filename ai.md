...
* **Use native Shopify subscriptions by default.** Most Shopify-triggered tasks should subscribe to regular `shopify/...` topics. Use [custom Shopify webhooks](platform/shopify/custom-webhooks.md) only when the task needs Shopify-side filtering, `include_fields`, metafield delivery, or metaobject events; those webhooks deliver onto `user/...` topics.
