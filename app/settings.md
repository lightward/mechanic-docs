...
The Settings page is organized into sections, accessed from the sidebar on the left (or a dropdown on smaller screens). The **General** and **User** sections cover the most common settings — the rest are for specific features you can explore as needed.

<figure><img src="../.gitbook/assets/mechanic_settings.jpg" alt="The Mechanic Settings page showing the General section"><figcaption></figcaption></figure>
...
Two different features in Mechanic share the word "webhooks." Quick decision: **data coming from Shopify? You probably want Custom Shopify webhooks. Data coming from anything else? You probably want Mechanic webhooks.**
...
Configure Shopify-side filters and payload customizations for any supported Shopify webhook topic, and route the deliveries onto a custom Mechanic event topic. Use this for filtered deliveries, slim payloads, Shopify-side metafield delivery, or [metaobject webhooks](../platform/shopify/custom-webhooks.md#when-to-reach-for-it) (which native subscriptions don't cover). See [Custom Shopify webhooks](../platform/shopify/custom-webhooks.md).
