...
* `event.shopify_topic` – on Shopify-sourced events (both native subscription deliveries and [Custom Shopify webhook](../../shopify/custom-webhooks.md) deliveries), this contains the original Shopify topic (e.g. `products/update`). It's `nil` for non-Shopify events.
