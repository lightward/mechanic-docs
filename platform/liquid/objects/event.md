...
* `event.parent` – if applicable, a reference to the event that used [an "event" action](../../../core-concepts/actions/types/event.md) to create this event; parents are available up to five generations deep \(e.g. `{{ event.parent.parent.parent.parent.parent }}`\), but no further
