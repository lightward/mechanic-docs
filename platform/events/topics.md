...
{% hint style="info" %}
**Need filtered deliveries, slimmer payloads, metafield filtering or delivery, or metaobject events?** The topics below are what Mechanic subscribes to natively, and they are the right default for most tasks. For advanced Shopify-side routing, configure a [custom Shopify webhook](../shopify/custom-webhooks.md) that re-delivers a supported Shopify topic onto a `user/...` topic, with Shopify-side filtering and payload customization. Custom Shopify webhooks also unlock `metaobjects/create`, `metaobjects/update`, and `metaobjects/delete` (with a required `type:` filter), which aren't in the native list below.
{% endhint %}

### Can Mechanic receive Shopify metaobject webhooks?

Yes, through [custom Shopify webhooks](../shopify/custom-webhooks.md#how-do-i-receive-shopify-metaobject-webhooks-in-mechanic). Native task subscriptions do not include metaobject topics, so `shopify/metaobjects/create`, `shopify/metaobjects/update`, and `shopify/metaobjects/delete` are not listed below. Configure them in **Settings → Custom Shopify webhooks**, give them a `user/...` Mechanic topic, and add the required `type:` filter. For a task-focused walkthrough, see [Subscribe to Shopify metaobject events](../../techniques/subscribe-to-shopify-metaobject-events.md).

### Native Shopify topics vs custom Shopify webhooks

The list below is the native `shopify/...` task subscription list. Custom Shopify webhooks use the supported topic picker in Settings, then deliver matching Shopify webhooks onto the `user/...` topic you choose. Use that picker as the source of truth for custom webhook topics.
...
* [Mechanic webhooks](../webhooks.md) may be configured to generate events using any User event topic
* [Custom Shopify webhooks](../shopify/custom-webhooks.md) deliver filtered or customized Shopify webhook events onto a User event topic
