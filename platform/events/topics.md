...
{% hint style="info" %}
**Need filtered deliveries, slimmer payloads, metafield delivery, or metaobject events?** The topics below are what Mechanic subscribes to natively. You can additionally configure a [Custom Shopify webhook](../shopify/custom-webhooks.md) that re-delivers any supported Shopify topic onto a `user/...` topic, with Shopify-side filtering and payload customization. Custom Shopify webhooks also unlock `metaobjects/create`, `metaobjects/update`, and `metaobjects/delete` (with a required `type:` filter), which aren't in the native list below.
{% endhint %}
