...
Looking for Shopify webhook topics? Jump straight to the [Shopify topic list](#shopify).
...
These are the native `shopify/...` topics that tasks can subscribe to directly. They are generated by Shopify webhooks, and are the right default for most Shopify-triggered tasks.
...
Need filtered deliveries, slimmer payloads, metafield filtering or delivery, or one-to-one routing to a `user/...` topic? Use [custom Shopify webhooks](../shopify/custom-webhooks.md).

Metaobject webhook topics are not part of Mechanic's native `shopify/...` subscription list. To receive `shopify/metaobjects/create`, `shopify/metaobjects/update`, or `shopify/metaobjects/delete`, use [custom Shopify webhooks](../shopify/custom-webhooks.md#how-do-i-receive-shopify-metaobject-webhooks-in-mechanic) with Shopify's required `type:` filter. For a task-focused walkthrough, see [Subscribe to Shopify metaobject events](../../techniques/subscribe-to-shopify-metaobject-events.md).
