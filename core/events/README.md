...
* **Custom events** — Developers can define their own event topics in the `user/` domain (e.g. `user/orders/export`). These are generated by tasks using the [Event action](../actions/event.md), via [Mechanic webhooks](../../platform/webhooks.md), or by [custom Shopify webhooks](../../platform/shopify/custom-webhooks.md), and are useful for chaining tasks together in multi-step workflows.
