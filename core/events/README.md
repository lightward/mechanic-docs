...
* **Custom events** — Developers can define their own event topics in the `user/` domain (e.g. `user/orders/export`). Tasks can generate them with the [Event action](../actions/event.md), which is useful for chaining tasks together in multi-step workflows. [Mechanic webhooks](../../platform/webhooks.md) can generate them from external HTTP requests, and [custom Shopify webhooks](../../platform/shopify/custom-webhooks.md) can generate them from filtered or customized Shopify webhook deliveries.
