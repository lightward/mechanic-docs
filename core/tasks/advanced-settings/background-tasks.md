...
* **Run this task in the background** – Background runs wait until your other tasks are caught up. A background task's runs only ever receive spare capacity: when any of your other work is waiting, that work goes first, and incoming events are processed ahead of background runs — with one narrow exception that keeps long-waiting background work alive (see below). Use this for bulk jobs and backfills that aren't time-sensitive.
